...
import { ScreenShot, VideoProfile, VideoPreset, MediaStatus } from '@graphql/types/generated-graphql-types';
      ...
      ffmpeg(mediaDir)
        ...
        .on('end', async () => {
          ...
          if (i < count) {
            ...
            await MediaModel.findByIdAndUpdate(mediaId, { screenshots: screenShotsPath, status });
            done();
          }
        })
        .screenshots(
  ...
  try {
    ...
    ffmpeg(videoPath)
      ...
      .on('end', async function (something) {
        ...
        await MediaModel.findByIdAndUpdate(mediaId, { status, videoProfiles });
